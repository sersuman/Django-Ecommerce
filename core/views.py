...
def add_to_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            order_item.save()
            messages.info(request,"This item quantity was updated to the cart")
...
def remove_single_item_from_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            messages.info(request,"This item was not in your the cart") 
            return redirect("core:order-summary")
    else:
...
class OrderSummaryView(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        try:
            ...
            messages.info(self.request,'You do not have active request')
            return redirect('/')
